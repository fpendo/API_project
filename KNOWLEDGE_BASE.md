...
- **Designo** — `/designo/` (SPA served from `/var/www/designo/`; API at `/designo/api/` proxied to `127.0.0.1:8620`, systemd `designo-backend`). **Motion Website Generator** (added 2026-07-05): questionnaire wizard → tagged photo upload → Claude generates a scroll-driven cinematic single-file website (GSAP/ScrollTrigger/Lenis via CDN + six signature effects: film grain, particles, vignette, glass cards, colour tints, scroll pacing).
  ...
  - **Dated-website funnel (2026-07-06):** Apify discovery gained `mode: "old_website"` (UK-pinned `countryCode: "gb"`) + a **South West sweep** (`POST /api/leads/sweep`: categories × 27 SW towns, services only — product categories + e-commerce markers excluded). Candidate sites go through `sources._audit_website` (technical age heuristics, threshold 4) then **`visual_audit.py`** — stealth Playwright screenshot (playwright-stealth 2.x; poll until sgcaptcha/Cloudflare challenges clear; auto-click cookie-consent Accept; networkidle wait; retry with 12s settle on "unknown") judged by Fable vision as a creative director: verdict `rebuild`/`borderline`/`modern` (skip)/`dead` (parking/not-published page = best lead)/`unknown`. Composite **opportunity score 0-100** (`main._opportunity_score`: dead/unreachable 100; else `(10−design)×8` + tech capped 20) on every lead payload; Leads board sorts worst-first with score chips. Leads with an existing site get `site_scraper.py` ground-truth scraping (homepage + 5 key pages → `raw.site_content`, stealth-rendered fallback for bot walls) before Fable writes the brief; `LEAD_BRIEF_SKILL`/`PITCH_EMAIL_SKILL` are audit-aware and pitch emails embed a plain-English "What we found on your current site" box. See vault note `Business/Website Audit Methodology.md`.
  - **GOTCHA — environment reset wipes uncommitted files (2026-07-06):** the VM reverted mid-session and every uncommitted file under `/opt/app` vanished (backend sources, frontend src, nginx `portal` config — restored from `deploy/nginx/portal.conf` + transcript — and `.env`, which had to be recreated by hand). SQLite DBs in `designo/storage/` survived. **Commit + push after every session.**
  - **GOTCHA — `pkill -f <script>` matches the calling shell** if the pattern appears in the command line — use `pgrep -f "patter[n]"` bracket trick.
