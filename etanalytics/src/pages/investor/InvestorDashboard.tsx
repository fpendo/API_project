  ...
  >
    ...
    <div className="card-glass rounded-2xl p-8 text-center">
      <h2 className="text-3xl font-bold mb-4">
        Build the Future of <span className="gradient-text">ETF Intelligence</span>
      </h2>
      <p className="text-xl text-gray-400 max-w-2xl mx-auto">
        Join the team building the ownership intelligence layer for €3 trillion in European ETFs.
        Early-stage equity. Category-defining product. Life-changing upside.
      </p>
    </div>
    ...
    <div className="card-glass rounded-2xl p-8">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-white/10">
              ...
              <th className="text-right py-3 px-4 text-gray-400 font-medium">Equity Range</th>
              <th className="text-right py-3 px-4 text-gray-400 font-medium">Conservative Exit</th>
              <th className="text-right py-3 px-4 text-gray-400 font-medium">Base Case Exit</th>
              <th className="text-right py-3 px-4 text-gray-400 font-medium">Aggressive Exit</th>
            </tr>
          </thead>
          <tbody className="text-gray-300">
            {[
              { role: 'CTO', equity: '1.5-2.5%', conservative: '£510K-£850K', base: '£1.2M-£1.9M', aggressive: '£2.3M-£3.8M' },
              { role: 'VP Sales', equity: '1.0-1.5%', conservative: '£340K-£510K', base: '£770K-£1.2M', aggressive: '£1.5M-£2.3M' },
              { role: 'VP Product', equity: '0.75-1.25%', conservative: '£255K-£425K', base: '£580K-£960K', aggressive: '£1.1M-£1.9M' },
              { role: 'Senior Engineer', equity: '0.25-0.5%', conservative: '£85K-£170K', base: '£190K-£385K', aggressive: '£375K-£755K' }
            ].map((row, i) => (
              <tr key={i} className="border-b border-white/5">
                <td className="py-3 px-4 font-medium">{row.role}</td>
                <td className="py-3 px-4 text-right text-primary-400">{row.equity}</td>
                <td className="py-3 px-4 text-right">{row.conservative}</td>
                <td className="py-3 px-4 text-right text-green-400 font-medium">{row.base}</td>
                <td className="py-3 px-4 text-right text-purple-400">{row.aggressive}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <p className="text-gray-500 text-xs mt-4">
        * Values shown are potential equity value at exit after 4-year vesting. Assumes dilution from Series A and B rounds.
      </p>
    </div>
    ...
    <div className="card-glass rounded-2xl p-8">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        {[
          { role: 'VP Engineering', location: 'London/Hybrid', equity: '1.0-1.5%', urgent: true },
          { role: 'Senior Backend Engineer', location: 'London/Remote', equity: '0.3-0.5%', urgent: true },
          { role: 'Senior Frontend Engineer', location: 'London/Remote', equity: '0.3-0.5%', urgent: false },
          { role: 'Enterprise Account Executive', location: 'London', equity: '0.2-0.4%', urgent: false }
        ].map((job, i) => (
          <div key={i} className="p-4 bg-dark-800 rounded-xl flex items-center justify-between">
            <div>
              <div className="flex items-center gap-2">
                <h4 className="font-medium">{job.role}</h4>
                {job.urgent && (
                  <span className="px-2 py-0.5 rounded text-xs bg-red-500/20 text-red-400">Urgent</span>
                )}
              </div>
              <p className="text-gray-500 text-sm">{job.location} • {job.equity} equity</p>
            </div>
            <a
              href="mailto:careers@etanalytics.co.uk"
      ...
      {[
        {
          ...
          description: 'Be part of defining a new category in financial data. No competition, just execution.'
        },
        {
          icon: TrendingUp,
          title: 'Massive Upside',
          description: 'Early-stage equity with 5-10x potential. Join before Series A closes.'
        },
        {
          icon: Shield,
          title: 'De-Risked',
          description: 'Funded through seed, strong pipeline, clear path to profitability.'
        }
      ].map((item, i) => (
        <div key={i} className="card-glass rounded-xl p-6">
          <div className="w-12 h-12 rounded-xl bg-primary-500/20 flex items-center justify-center mb-4">
    ...
    {/* CTA */}
    <div className="text-center p-8 bg-gradient-to-r from-primary-500/20 to-accent-500/20 rounded-2xl border border-primary-500/30">
      <h3 className="text-2xl font-bold mb-4">Ready to Build Something Big?</h3>
      <p className="text-gray-400 mb-6">
        The best time to join a startup is before the world knows about it.
      </p>
      <a
        href="mailto:careers@etanalytics.co.uk"
