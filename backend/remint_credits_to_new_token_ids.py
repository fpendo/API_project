"""
Re-mint credits to the new SchemeNFT token IDs (1, 2, 3) to match the updated database.
This is needed after SchemeNFTs were re-minted and database was updated.
"""
import os
...
from app.models import Scheme, Trade, Account, AccountRole
...
def remint_credits():
    """Re-mint credits to new token IDs"""
    db = SessionLocal()
    try:
        ...
        print("=" * 60)
        print("RE-MINTING CREDITS TO NEW TOKEN IDs")
        print("=" * 60)
        print("")
        ...
        # Group trades by scheme (using new token IDs from database)
        scheme_totals = {}
        for trade in trades:
            ...
            scheme_id = scheme.nft_token_id  # This is now the new token ID (1, 2, 3)
        ...
        print("Checking balances and minting to new token IDs:")
        print("")
        ...
        for scheme_id, data in scheme_totals.items():
            ...
            if current_balance < required_credits:
                ...
                print(f"  [MINTING] {credits_to_mint:,} credits to token ID {scheme_id}...")
        ...
        print("SUMMARY")
        print("=" * 60)
        print("Credits have been re-minted to the new token IDs.")
