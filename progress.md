...
### 2026-07-05 (night) — Follow-up pack (proposal + Stripe) & Mailbox

- **Follow-up pack for interested prospects** (`app/proposal.py` + routes in `prospect.py`): session-gated `/p/{slug}/proposal/` page — dark/gold editorial design with the prospect's scroll GIF in a browser frame, three pillars, £695/£59 pricing cards with itemised inclusions, shadow-site USP callout, a **sample weekly SEO report** (metric cards, SVG sparkline, top-queries table personalised by category/town, technical health checklist, action-of-the-week — clearly labelled SAMPLE), two-step payment CTAs and a 6-question FAQ. `proposal_viewed` event logged (deduped hourly). Pricing panel injected into prospect sites now links to the proposal page.
- **Stripe payments** (`app/payments.py`): one-off £695 Checkout (card, `mode=payment`) + £59/month Checkout (`mode=subscription`, **BACS direct debit + card**). Monthly Product/Price auto-created on first use, cached in settings (`stripe_monthly_price_id`, env override `STRIPE_MONTHLY_PRICE_ID`). Sessions carry `lead_id` in metadata — **this is how a payment is tied to the right website/lead**. Webhook `POST /api/payments/webhook` (signature-verified) flips the lead to `won` on `checkout.session.completed` and logs `payment_received`/`subscription_created`/`invoice_payment_failed`/`subscription_cancelled` events. Feature hidden until `STRIPE_SECRET_KEY` set; `payments_enabled` exposed in `/api/config`. `stripe` lib in venv (15.3.0). Thank-you page after payment.
- **Mailbox module** (`app/mailbox.py`, `mail_messages` table, `Mailbox.tsx` at `/designo/mailbox`): unified inbox/sent for the outreach address. **Inbound**: IMAP poller (any provider, creds stored in settings via UI, 2-min background thread + manual "Check now", UID-cursor incremental fetch, skips historic mail on first connect, dedupes by Message-ID) — replies matched to leads by sender address, `email_reply_received` event logged. **Outbound**: replies sent via Resend with In-Reply-To/References threading; approved pitch emails are mirrored into sent items. **"Insert package follow-up"** button drops in the canned pricing response (same price for all; only proposal URL + login creds are per-lead — payment linkage comes from the proposal page's Stripe metadata). Two-pane UI: thread list with unread badges + conversation view with reply composer; IMAP settings panel with save-and-test.
- **Pricing settings corrected** from stale defaults (£495/£39) to the confirmed **£695 one-off / £59 per month** (DB + `DEFAULT_SETTINGS`).
- Discovered `designo-backend` runs under systemd (`designo-backend.service`, auto-restarts) — use `systemctl restart designo-backend`, not manual kill/nohup.

### 2026-07-05 (late night) — Welcome pack, public landing & admin login

- **Welcome pack** (`app/welcome.py`): sent automatically by the Stripe webhook when the launch payment clears (idempotent — checks `welcome_sent` event first; failures are non-fatal and logged as `welcome_send_failed`). Dark-header HTML email + matching session-gated web page `/p/{slug}/welcome/`. Contents: payment confirmed, numbered next-steps timeline (choose domain → we register & go live in 24h → Search Console/indexing → first Monday report), "the one thing we need from you" box (reply with 1st/2nd choice domain name), and a direct-debit CTA that only appears if no subscription exists yet. Mirrored into Mailbox sent items. Manual **Send/Re-send welcome pack** + **Preview welcome pack** buttons on won leads (`POST /api/leads/{id}/send-welcome`, `GET .../welcome-preview`).
- **Domain journey decision**: domain cost is absorbed, never itemised — first year inside the £695, renewals inside the £59/month, so the prospect never sees a third bill. We register on their behalf after payment (they reply with name choices); if they own one already we connect it. Proposal card/FAQ + mailbox follow-up template updated to match.
- **Public landing page** (`Home.tsx` at `/designo/`): "Beautiful websites, ready for what's coming" hero, three pillars (cinematic by default / built for the agent economy / provably visible on Google), founder section — **Angie Pendarves**: 15 years at Cheil (Samsung's global marketing agency) as senior account director, built Samsung's concierge application, ran Colombia's first digital election campaign for President-elect Juan Manuel Santos. No mention of the systematic pipeline. Invitation-only closing note. Login button top-right.
- **Dogfooded landing page (LIVE)**: `/designo/` now serves a Designo-generated cinematic site (project `5087a7de463a4c13b3bfa4b31a44c8a6`, hand-crafted studio brief: agent-economy manifesto, "title sequence" concept with REEL-A timecode motif and JSON-LD snippets as design elements, Angie founder section, "by invitation" close, model included the `/designo/login` link itself). The photo-requirement check on `/generate` was removed (zero photos → AI artwork commissioning, same as lead mockups — 8 artworks commissioned). Nginx exact-match `location = /designo/` serves `/var/www/designo/landing/index.html` (falls back to the SPA index if missing); all other `/designo/*` routes still serve the studio SPA; assets + shadow files copied to the web root so `/designo/llms.txt`, `/designo/agent.json`, `/designo/agent.html` resolve. Redeploy anytime with `bash /opt/app/designo/deploy_landing.sh <project_id>`.
- **AI-vs-search stats added** (researched 2025-26 data: ChatGPT 800M weekly users / ~2bn questions a day; ~50% of Google searches open with an AI answer reaching 2bn+ people/month; organic clicks fall ~60% when it appears and 68% of searches end with no click (Seer/SparkToro); AI-referred traffic grew ~1,200% in a year and converts at 2.5× Google organic (Chartbeat/Similarweb)). Added to (1) the **proposal/follow-up pack** as a "Why now — the numbers" stat-card section with a "what this means for you" note tying it to the shadow site, with a small-print sources line; (2) the **generated landing page** via a pipeline iteration — Fable produced a "THE SHIFT — The numbers don't whisper. They roar." section with gold Fraunces numerals, mono captions and count-up reveals; redeployed with `deploy_landing.sh`.
- **Designo Obsidian vault created** at `/opt/app/vaults/Designo/` (same pattern as NEMX/ETAnalytics/Jarvis — open as a standalone vault in Obsidian): `Home.md` index + Architecture (Overview, Generation Pipeline, Backend, Frontend & Portal, Deployment), Business (Client Journey, Lead-Gen Engine, Pricing & Payments, SEO + ASO Report, Agent Economy Positioning) and Operations (Known Bugs & Fixes, Dev Workflow). Full write-up of everything built to date.
- **ASO added to the weekly report**: the report is now the **weekly SEO + ASO report** everywhere. New "Agent visibility — ASO" section in the sample report (`proposal.py`): AI crawler visits (GPTBot / Google-Extended / PerplexityBot / ClaudeBot counts), visitors referred by AI assistants (with the 2.5× conversion note), and a 3/3 shadow-site health check (llms.txt served / agent.json valid / JSON-LD valid). All of this is genuinely measurable once sites are live — crawler user-agents and AI referrers appear in nginx logs, and shadow files can be health-checked. Copy renamed to "SEO + ASO" across the proposal (pillar 03, pricing list + new "AI-agent visibility tracking" line, section intro, FAQ answer explaining the two halves), welcome pack (step 4 + monthly-plan blurb + web page), mailbox follow-up template, Stripe thank-you copy, Documents tab descriptions, and the deployed landing shadow files (`agent.json`/`agent.html`/JSON-LD in `landing/index.html`).
- **Documents tab** (`Documents.tsx` at `/designo/documents`, nav link next to Mailbox): previews of the client-facing packs rendered with sample data (Harper & Sons Roofing) — (1) follow-up pack / proposal incl. the sample weekly SEO report, (2) welcome pack email, plus a toggle showing the canned package follow-up email text. Backend: authed `GET /api/documents/proposal|welcome` (HTML) and `/api/documents/followup-email` (JSON).
- **Root-caused the malformed-JSON failures**: the concept stage capped Claude at `max_tokens=8000`; artwork-heavy photo-less briefs (6-10 detailed artwork commissions) truncate mid-JSON at that limit — the "Expecting ',' delimiter at char ~19-22k" errors were cut-off output, not model noise. Raised to 16000, bumped `call_claude_json` to 3 attempts, and it now dumps failed raw responses to `/tmp/designo_bad_json_*.txt` for diagnosis. First attempt after the fix parsed cleanly.
- **Admin auth** (`app/auth.py`): single-operator password login (`POST /api/auth/login` → HMAC-signed 30-day cookie `designo_admin`). HTTP middleware now protects **all** `/api/*` routes (previously wide open!) except auth endpoints, `/api/config`, both webhooks (signature-verified anyway) and `/api/preview/*` (Playwright capture hits localhost cookieless; project IDs are unguessable). Frontend: `/login` page, `RequireAuth` guard around all studio routes (`/start`, `/create`, `/project/*`, `/sites`, `/leads`, `/mailbox`), logout button in Shell. Password: `DESIGNO_ADMIN_PASSWORD` env or `admin_password` setting; auto-generated on first boot and printed to the log. Internal Landing moved from `/` to `/start`.

### 2026-07-06 — Dated-website funnel, visual audit & worst-first leads board

> **Environment reset note:** the VM was reverted mid-session — all uncommitted files under `/opt/app` were lost (backend `app/*.py` sources, frontend `src/`, nginx `portal` config, `.env`, vault additions, this file's entries for the lead-audit session). Everything was restored from git plus rebuilt from the running process/`__pycache__`/chat transcript; `.env` recreated (Anthropic, fal, GitHub, Apify keys). The SQLite DBs live in `designo/storage/` and survived. **Lesson: commit + push after every session.**

- **Apify pinned to UK** (`countryCode: "gb"`) after a "plumbers in Truro" run returned Truro, Nova Scotia.
- **Dated-website discovery mode** (`mode: "old_website"`): Apify runs with `website: withWebsite`; every candidate homepage is fetched and scored by `_audit_website` for technical age signals (no viewport +4, stale copyright up to +4 by age, no HTTPS +3, framesets/marquee/font/center +3, Flash +3, legacy generator +3, jQuery 1.x +2, table-era markup +2, no structured data +1; threshold 4). Unreachable sites import immediately as "likely abandoned". Findings stored in `raw.site_audit`. Target dropdown (No website / Dated website) in the Leads discovery panel.
- **South West sweep** (`start_region_sweep`, `POST /api/leads/sweep`, "South West sweep" source in UI): category × 27 SW towns fan-out, dated-website audit, **services only** — product/retail excluded by Maps-category blocklist + e-commerce markers (cart/Shopify/WooCommerce) in the HTML. ≤60 searches per sweep; polling deadline scales with query count.
- **Creative-director visual audit** (`visual_audit.py`) — the qualifying judgement. Technical signals can't tell SBR Electrical (screams 2005) from TWR Lighting (technically old, visually fine). Stealth Playwright screenshot (playwright-stealth 2.x, realistic UA, en-GB, 4s settle for Cloudflare checks) → Fable vision judges design 1-10 as of 2026 → verdict `rebuild` (1-3, bread-and-butter) / `borderline` (4-5) / `modern` (6-10, skipped) / `dead` (not-published/parking/error page — best lead of all) / `unknown` (bot-check/blank — no guessing, returns None). Verdict overrides the technical score for import decisions; `status_detail` carries "REBUILD — design 2/10, reads as 2004-2009…".
- **Composite opportunity score** (`main._opportunity_score`, 0-100 on every lead payload): unreachable/dead = 100; else `(10 − design_score) × 8` + technical score capped at 20; visual-not-yet-reviewed sits mid-table at 20 + technical. **Leads board sorts worst-first** (biggest opportunities on top) with a colour-coded score chip (red ≥60 / amber ≥40 / slate); unaudited no-website leads follow, newest first.
- **Lead detail restructured**: "Opportunity rating" card (big score + visual/technical breakdown + creative-director reasons + pitch line) sits **above** the Business card; Business card now shows the **website link**; below it a "Their current website content" card lists the scraped pages as expandable sections (title, char count, full text) — this is the ground truth Fable uses.
- **Existing-site content scraping** (`site_scraper.py` + `lead_agent._ensure_site_content`): before the brief is written, leads with a website get homepage + up to 5 key internal pages (about/services/contact/testimonials…) scraped into `raw.site_content`; `LEAD_BRIEF_SKILL` treats it as **ground truth** (real services/team/accreditations/testimonials, improved writing). Sites that block plain fetches (Cloudflare "just a moment" etc.) retry through stealth headless Chromium. `site_content_scraped` event logged.
- **Audit-aware outreach**: `PITCH_EMAIL_SKILL` distinguishes NO WEBSITE vs DATED WEBSITE prospects (respectful "your work deserves better than your website" tone); the HTML shell embeds a "What we found on your current site" box — creative-director observations first, technical findings translated to plain English (`_FINDING_TRANSLATIONS`); footer reason adapts.
- **Re-triage of existing leads**: original run (97 leads pre-stealth): 11 rebuild / 29 borderline / 20 modern (skipped) / 33 failed on bot checks. Failed ones re-run with stealth + the new `dead` verdict; qualified leads back-filled with scraped site content (`/tmp/retry_visual.py`, log `/tmp/retry_visual.log`).
- **Vault updated**: new [[Website Audit Methodology]] note (both audit stages, weights, verdicts, opportunity-score formula, what's stored, how findings reach the prospect); Lead-Gen Engine note gained dated-website/sweep sourcing, prioritisation and ground-truth-scraping sections; linked from Home.
- **Deps**: `playwright-stealth>=2.0`, `beautifulsoup4>=4.12` added to requirements.
- **Playbook page** (`Playbook.tsx` at `/designo/playbook`, nav link next to Documents): the whole system on one page — a colour-coded visual pipeline diagram (4 phases × 3 stages: Find & qualify → Build the pitch asset → Pitch & converse → Close & run, with connecting arrows and phase-gate labels) plus a thorough write-up: audit methodology and verdicts, opportunity-score formula, ground-truth scraping, human approval gate, prospect journey, Stripe/payment linkage, domain policy, weekly SEO + ASO report, key facts table, and the agent-economy positioning.
